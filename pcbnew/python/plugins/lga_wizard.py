...
class LGAWizard(HFPW.HelpfulFootprintWizardPlugin):
    ...
    def GetDescription(self):
        return "LGA footprint wizard"

    def GetValue(self):
        return "LGA"

    def GenerateParameterList(self):
        self.AddParam("Pads", self.pad_num_pads_vert_key, self.uNatural, 10)
        self.AddParam("Pads", self.pad_num_pads_horz_key, self.uNatural, 6)
        self.AddParam("Pads", self.pad_width_key, self.uMM, 0.25)
        self.AddParam("Pads", self.pad_length_key, self.uMM, 0.35)
        self.AddParam("Pads", self.pad_pitch_key, self.uMM, 0.5)
        self.AddParam("Pads", self.pad_row_spacing_key, self.uMM, 2.45)
        self.AddParam("Pads", self.pad_handsolder_key, self.uMM, 0)
        
        self.AddParam("Body", self.body_width_key, self.uMM, 3.15)
        self.AddParam("Body", self.body_length_key, self.uMM, 3.15)
    ...
    def CheckParameters(self):
        self.CheckParamInt(
            "Pads", '*' + self.pad_num_pads_vert_key,
            is_multiple_of=2)
        self.CheckParamInt(
    ...
    def BuildThisFootprint(self):
        ...
        array = PA.PadLineArray(self.GetPad(90), pad_num_vert_pads/2, pad_pitch, True, pin1Pos)
        ...
        array.SetFirstPadInArray(int(pad_num_vert_pads/2+1))
        array.AddPadsToModule(self.draw)

        #right row
        pin1Pos = pcbnew.wxPoint(pad_row_spacing/2+pad_handsolder/2, 0)
        array = PA.PadLineArray(self.GetPad(90), pad_num_vert_pads/2, -pad_pitch, True, pin1Pos)
        array.SetFirstPadInArray(int(pad_num_vert_pads/2+pad_num_horz_pads/2+1))
        array.AddPadsToModule(self.draw)

        #top row
        pin1Pos = pcbnew.wxPoint(0, -pad_row_spacing/2-pad_handsolder/2)
        array = PA.PadLineArray(self.GetPad(), pad_num_horz_pads/2, -pad_pitch, False, pin1Pos)
        array.SetFirstPadInArray(int(pad_num_vert_pads+pad_num_horz_pads/2+1))
        array.AddPadsToModule(self.draw)
